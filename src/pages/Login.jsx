...
//         <h2 className="text-2xl font-bold mb-4 text-center text-burgundy-600">Login</h2>
...
//         navigate("/");
//       } else {
//         setErr(data.message || "Google login failed");
//       }
//     } catch (err) {
//       console.error(err);
...
//         <h2 className="text-3xl font-extrabold mb-6 text-center text-burgundy-600">Login</h2>
...
//           <button className="bg-burgundy-600 hover:bg-gold-600 text-white py-3 rounded-lg font-semibold transition-all transform hover:scale-105 hover:shadow-xl">Login</button>
//         </form>
...
//         <p className="mt-6 text-center text-gray-700">
//           Don’t have an account?{" "}
//           <span onClick={() => navigate("/signup")} className="text-burgundy-600 font-semibold cursor-pointer hover:underline">Sign up</span>
//         </p>
//       </div>
//     </div>
//   );
...
export default function Login() {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      const data = await res.json();
      if (res.ok) {
        setToken(data.token);   // <-- use context setter instead of localStorage directly
        navigate("/");
      } else setErr(data.message || "Login failed");
    } catch (err) {
      console.error(err);
  ...
  return (
    <div className="h-screen flex items-center justify-center bg-gradient-to-br from-gold-600 via-gray-100 to-burgundy-600 overflow-hidden p-0">
      <div className="w-full max-w-sm bg-white/90 p-8 rounded-2xl shadow-2xl 
                      transform transition duration-500 hover:scale-105 hover:shadow-3xl">
        <h2 className="text-3xl font-extrabold mb-6 text-center text-burgundy-600 tracking-wide">Login</h2>

        {err && <div className="text-red-600 mb-4 text-center animate-pulse">{err}</div>}

        <form onSubmit={handleLogin} className="flex flex-col gap-5">
          <input
            className="border border-gray-300 p-3 rounded-lg focus:outline-none focus:ring-2 focus:ring-gold-600 transition duration-300"
          ...
          />
          <button className="bg-burgundy-600 hover:bg-gold-600 text-white py-3 rounded-lg font-semibold transition-all duration-500 ease-in-out transform hover:scale-105 hover:shadow-xl">Login</button>
        </form>
